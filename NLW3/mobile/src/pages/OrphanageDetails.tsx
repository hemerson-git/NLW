...
export default function OrphanageDetails() {
  ...
  const { id } = route.params as OrphanageRouteId;
  ...
  if (!orphanage) {
    return (
      <View style={styles.containerLoading}>
        <ActivityIndicator size='small' color='#15b6d6'/>
      </View>
    )
  }
      ...
      <View style={styles.imagesContainer}>
        <ScrollView horizontal pagingEnabled>
          {orphanage.images.map(image => {
            const url = image.url.replace('localhost', '192.168.0.104');
            return <Image key={image.id} style={styles.image} source={{ uri: url }} />
          })}
        </ScrollView>
      </View>
